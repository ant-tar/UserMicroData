...
class usermicrocardPackage
{
    ...
    /**
     * usermicrocardPackage constructor.
     *
     * @param $core_path
     * @param array $config
     */
    ...
    {
        /** @noinspection PhpIncludeInspection */
        $types = include($this->config['elements'] . 'content_types.php');
        if (!is_array($types)) {
        ...
        $attributes = [
            xPDOTransport::UNIQUE_KEY => array('name'),
            xPDOTransport::PRESERVE_KEYS => true,
            xPDOTransport::UPDATE_OBJECT => !empty($this->config['update']['content_types']),
            xPDOTransport::RELATED_OBJECTS => false,
        ];
        foreach ($types as $name => $data) {
            ...
            $type->fromArray(array_merge([
                ...
                'file_extensions' => $this->config['ext'],
                'mime_type' => $this->config['type'],
            ], $data), '', true, true);
            $vehicle = $this->builder->createVehicle($type, $attributes);
            $this->builder->putVehicle($vehicle);
        }
        $this->modx->log(modX::LOG_LEVEL_INFO, 'Packaged in ' . count($types) . ' Content Type');
    }
...
$install = new usermicrocardPackage(MODX_CORE_PATH, $config);
